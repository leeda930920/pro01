...
<body>
	...
	<%
		if(rs.next()){
			//이미 회원이 사용중이므로 가입할 수 없는 아이디
	%>
			<h3>사용이 불가능한 아이디입니다. 다시 입력하시기 바랍니다.</h3>
			<input type="hidden" id="ps" name="ps" value="no">
			<input type="hidden" id="msg" name="msg" value="사용이 불가능한 아이디입니다. 다시 입력해주시길 바랍니다.">
	<%		
	} else {
	//현재 존재하지 않는 아이디로서 사용이가능한 아이디
	%>
			<h3>사용이 가능한 아이디입니다.</h3>
			<input type="hidden" id="ps" name="ps" value="yes">
			<input type="hidden" id="msg" name="msg" value="사용이 가능한 아이디입니다.">
	<%} %>
